...
StreamPipe::StreamPipe(bool useOSPipe)
{
    if(useOSPipe)
    {
        int fds[2];
        pipe(fds);
        FILE * reader = fdopen(fds[0], "rb");
        FILE * writer = fdopen(fds[1], "wb");
        readerInternal = shared_ptr<Reader>(new FileReader(reader));
        writerInternal = shared_ptr<Writer>(new FileWriter(writer));
    }
    else
    {
